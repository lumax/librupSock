...
{
  _pollMngServer_t * pollServer = (_pollMngServer_t*)dat;
  ...
  ec_neg1( write(pollServer->pPollSrc->fd, "Server!\n", 9 ) ) 
  return 0;
  EC_CLEANUP_BGN
    return -1;
  EC_CLEANUP_END
...
static _pollMngServer_t ServerPollSrc1 = {
  .pPollSrc = &PollSrc.Srcs[1],
};
...
int main(void)
{
  ...
  printf("server enter poll\n");
      pollMngPoll();
 printf("server leaving poll\n");
    /*
